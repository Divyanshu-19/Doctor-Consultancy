...
let handleMessage = async (sender_psid, received_message) => {
    ...
    if (received_message.sticker_id) {
        await callSendAPI(sender_psid, " Doctors Consultancy !!!");
        return;
    }
...
let handleEntity = async (name, sender_psid, entity) => {
    switch (name) {
        ...
        case"booking":
            ...
            await callSendAPIv2(sender_psid, "Đặt lịch khám bệnh", "Hướng dẫn đặt lịch khám bệnh tại Doctors Consultancy", BOOKING_IMAGE_URL, BOOKING_URL);
            await sendMessageDefault(sender_psid, "Xem thêm thông tin:");
            break;
        case"info":
            await callSendAPI(sender_psid, "Bạn đang tìm hiểu về thông tin website, xem thêm ở link bên dưới nhé.");
            await callSendAPIv2(sender_psid, "Thông tin website", "Thông tin website Doctors Consultancy", INFOWEBSITE_IMAGE_URL, INFOWEBSITE_URL);
            await sendMessageDefault(sender_psid, "Xem thêm thông tin:");
            break;
        default:
            ...
            await callSendAPIv2(sender_psid, "Hỗ trợ khách hàng", "Thông tin hỗ trợ khách hàng Doctors Consultancy", DEFAULT_IMAGE_URL, DEFAULT_URL);
            await sendMessageDefault(sender_psid, "Xem thêm thông tin:");
    }
};
